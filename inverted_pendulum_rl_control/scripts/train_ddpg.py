...
class TrainDDPG:
    def __init__(self):
        ...
        self.joint_sub = rospy.Subscriber('/rrbot/joint_states', JointState, self.joint_callback)
        self.action_pub = rospy.Publisher('/rrbot/joint1_effort_controller/command', Float64, queue_size=10)
    ...
    def reset(self):
        ...
        try:
            resp = rospy.ServiceProxy('/gazebo/set_model_configuration', SetModelConfiguration)(req)
            if not resp.success:
    ...
    def get_reward(self, state_next):
        ...
        reward += 20*np.cos(angle)
        # if np.abs(angle) < 0.05:
        #     reward += 100
        # elif np.abs(angle) < 0.2:
        #     reward += 10
        # elif np.abs(angle) > 2.9:
        #     reward -= 20
        reward += 60*np.exp(-np.abs(angle)) - 30
        reward += 10*np.exp(-np.abs(state_next[1]))
        reward += 10*np.exp(-np.abs(state_next[2]))

        if np.abs(state_next[1]) > 10:
            reward -= 20
    ...
    def train(self):
        ...
        for episode in range(500):
            ...
            rospy.loginfo('Episode: %d, score: %f, running_reward: %f, running_q: %f' % (episode, score, running_reward, running_q))
            rospy.loginfo('Max action: %f, min action: %f' % (max_action, min_action))
